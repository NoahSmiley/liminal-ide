...
export function WelcomeScreen({ onCommand, onTutorial }: WelcomeScreenProps) {
  ...
  return (
    <div className="flex-1 overflow-y-auto flex flex-col items-center justify-center gap-12 py-16">
      <div className="flex flex-col items-center gap-4">
        <LiminalLogo size={64} className="opacity-30" />
      ...
      {projects.length > 0 && (
        <div className="w-full max-w-sm">
          ...
          <div className="flex flex-col gap-0.5 border border-panel-border/60 rounded-[3px] overflow-hidden">
              ...
              <div key={ws ?? "__none"}>
                ...
                {items.map((p) => (
                  <button
                    ...
                    className="flex items-center justify-between px-3 py-2.5 hover:bg-card/60 transition-colors text-left border-b border-border/30 last:border-0 w-full"
                  >
                    <span className="text-zinc-300 text-[13px]">{p.name}</span>
